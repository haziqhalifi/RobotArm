...
const unsigned long FK_PRINT_INTERVAL_MS = 500;
...
// Initial theoretical dimensions. Measure the real arm and tune these values
// during calibration so the FK output matches the physical end-effector.
constexpr float BASE_HEIGHT_MM = 70.0f;
constexpr float SHOULDER_LINK_MM = 105.0f;
constexpr float ELBOW_LINK_MM = 105.0f;
constexpr float WRIST_LINK_MM = 70.0f;

void printStatus();
int clampJointPulse(int jointIndex, int us);
int pulseToDegrees(int jointIndex, int us);
ArmPose calculateForwardKinematics();
void printForwardKinematics();
void printForwardKinematicsPeriodically();
...
ArmPose calculateForwardKinematics() {
    const float baseRad = radians((float)pulseToDegrees(0, joints[0].currentUs));
    const float shoulderRad =
        radians((float)pulseToDegrees(1, joints[1].currentUs) - 90.0f);
    const float elbowRad =
        radians((float)pulseToDegrees(2, joints[2].currentUs) - 90.0f);
    const float wristPitchRad =
        radians((float)pulseToDegrees(3, joints[3].currentUs) - 90.0f);

    const float upperArmAngle = shoulderRad;
    const float forearmAngle = shoulderRad + elbowRad;
    const float wristAngle = forearmAngle + wristPitchRad;

    const float planarReach =
        SHOULDER_LINK_MM * cosf(upperArmAngle) +
        ELBOW_LINK_MM * cosf(forearmAngle) +
        WRIST_LINK_MM * cosf(wristAngle);
    const float height =
        BASE_HEIGHT_MM +
        SHOULDER_LINK_MM * sinf(upperArmAngle) +
        ELBOW_LINK_MM * sinf(forearmAngle) +
        WRIST_LINK_MM * sinf(wristAngle);

    ArmPose pose;
    pose.x = planarReach * cosf(baseRad);
    pose.y = planarReach * sinf(baseRad);
    pose.z = height;
    pose.rx = (float)pulseToDegrees(4, joints[4].currentUs);
    pose.ry = (float)pulseToDegrees(3, joints[3].currentUs);
    pose.rz = (float)pulseToDegrees(0, joints[0].currentUs);
    return pose;
}

void printForwardKinematics() {
    const ArmPose pose = calculateForwardKinematics();
    Serial.printf("X: %.0f mm, Y: %.0f mm, Z: %.0f mm | Rx: %.0f, Ry: %.0f, Rz: %.0f |\n",
                  pose.x, pose.y, pose.z, pose.rx, pose.ry, pose.rz);
}

void printForwardKinematicsPeriodically() {
    const unsigned long now = millis();
    if (now - lastFkPrintAt < FK_PRINT_INTERVAL_MS) {
        return;
    }
...
void loop() {
    ...
    printForwardKinematicsPeriodically();
}
